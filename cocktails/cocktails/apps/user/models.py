...
class User(BaseModel, AbstractUser):
    ...
    roles = ChoiceArrayField(models.CharField(
        choices=ROLES_CHOICES,
        default='Customer'
    ), default=list, null=True, blank=True, verbose_name='Роли')
    ...
    jwt_updated_at = models.DateField(null=True, blank=True, verbose_name='Дата обновления по JWT токену',
                                      help_text='Если в ссылке для авторизации по JWT токену приходит дата позже '
                                                'этой, то обновляем личные данные пользователя')
